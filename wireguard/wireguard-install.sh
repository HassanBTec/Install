...
function installQuestions() {
	...
	SERVER_PUB_IP=aedu.uaenorth.cloudapp.azure.com

	#SERVER_PUB_IP=$(ip -4 addr | sed -ne 's|^.* inet \([^/]*\)/.* scope global.*$|\1|p' | awk '{print $1}' | head -1)
	SERVER_WG_IPV4=$(ip -4 addr show wg0 | grep -oP '(?<=inet\s)\d+(\.\d+){3}')

	SERVER_WG_IPV6=$(ip -6 addr show wg0 | grep -oP '(?<=inet6\s)[\da-f:]+')
	...
	until [[ ${SERVER_WG_IPV6} =~ ^([a-f0-9]{1,4}:) ]]; do
	...
	read -n1 -r -p "Press any key to continue to install or configure clients..."
}
